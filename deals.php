<?php
    ...
    $stmt = $pdo->prepare('SELECT p.* FROM product p,category c 
                            WHERE p.c_id=c.c_id AND p.rrp!=0 
    ...
    $total_products = $pdo->query('SELECT p.* FROM product p,category c WHERE p.c_id=c.c_id AND p.rrp!=0;')->rowCount();
?>
<?=template_header('deals')?>
        <!-- FRUIT ITEMS -->
        <div class="container">
           <div class="top">
                <h2 id="name">Deals</h2>
            ...
            <div class="row">
                ...
                <div class="fruit-items">
                    ...
                    <div class="description">
                        <h4 style="color: red; padding-bottom: 5px; "><?=$product['name']?></h4>
                ...
                <?php if(isset($_SESSION['sess_user_id']) && $_SESSION['sess_user_id'] != ""):?>
                    ...
                    <a href="ppl.php?page=deals&p=<?=$current_page-1?>">Prev</a>
                    <?php endif; ?>
                    <?php if ($total_products > ($current_page * $num_products_on_each_page) - $num_products_on_each_page + count($products)): ?>
                    <a href="ppl.php?page=deals&p=<?=$current_page+1?>">Next</a>
                    <?php endif; ?>
                    <?php else:?>
                        <?php if ($current_page > 1): ?>
                        <a href="index.php?page=deals&p=<?=$current_page-1?>">Prev</a>
                        <?php endif; ?>
                        <?php if ($total_products > ($current_page * $num_products_on_each_page) - $num_products_on_each_page + count($products)): ?>
                        <a href="index.php?page=deals&p=<?=$current_page+1?>">Next</a>
                        <?php endif; ?>
                <?php endif; ?>
                </div> 
            </div>
