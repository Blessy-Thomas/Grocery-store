...
<?php
    ...
    $stmt = $pdo->prepare('SELECT p.* FROM product p,category c 
                            WHERE p.c_id=c.c_id AND p.c_id=2 
    ...
    $total_products = $pdo->query('SELECT p.* FROM product p,category c WHERE p.c_id=c.c_id AND p.c_id=2;')->rowCount();
?>
<?=template_header('fruits')?>
        <!-- FRUIT ITEMS -->
        <div class="container">
            <div class="top">
            ...
            <div class="row">
                ...
                <?php if(isset($_SESSION['sess_user_id']) && $_SESSION['sess_user_id'] != ""):?>
                    ...
                    <a href="ppl.php?page=fruits&p=<?=$current_page-1?>">Prev</a>
                    <?php endif; ?>
                    <?php if ($total_products > ($current_page * $num_products_on_each_page) - $num_products_on_each_page + count($products)): ?>
                    <a href="ppl.php?page=fruits&p=<?=$current_page+1?>">Next</a>
                    <?php endif; ?>
                    <?php else:?>
                        <?php if ($current_page > 1): ?>
                        <a href="index.php?page=fruits&p=<?=$current_page-1?>">Prev</a>
                        <?php endif; ?>
                        <?php if ($total_products > ($current_page * $num_products_on_each_page) - $num_products_on_each_page + count($products)): ?>
                        <a href="index.php?page=fruits&p=<?=$current_page+1?>">Next</a>
                        <?php endif; ?>
                <?php endif; ?>
                </div> 
            </div>
